...
- **5 pluggable vault backends** — HashiCorp Vault (KV v2), AWS Secrets Manager, GCP Secret Manager, Azure Key Vault, Internal ADK Vault
...
- **Feature-flagged backends** — compile only the backends you need (`hashicorp`, `aws`, `gcp`, `azure`, `adk-vault`)
